...
{
  constructor(
    protected readonly service: CrudAndAuditOperationsService<
      Entity,
      EntityToAudit,
      ID
    >,
  ...
  public async findOneEntityLogsWithPaginator(
    ...
    try {
      const anEntityLogs = await this.service.findOneEntityLogsWithPaginator(
        id,
        Number(page),
        Number(limit),
      );
  ...
  public async findOneEntityLogs(
    ...
    try {
      const anEntityLogs = await this.service.findOneEntityLogs(id);
  ...
  public async findEntityLogsWithPaginator(
    ...
    try {
      const entityLogs = await this.service.findEntityLogsWithPaginator(
        Number(page),
        Number(limit),
      );
  ...
  public async findEntityLogs(
    ...
    try {
      const entityLogs = await this.service.findEntityLogs();
  ...
  public async undoLastChange(@Param('id') id: ID, @Res() res?: Response) {
    try {
      const undoChangeResult = await this.service.undoLastChange(id);
